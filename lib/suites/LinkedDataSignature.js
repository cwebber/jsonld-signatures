...
module.exports = class LinkedDataSignature {
  ...
  async sign(input, options) {
    ...
    if (options.proofPurpose) {
      proof.proofPurpose = options.proofPurpose.uri;
      options.proofPurpose.addFieldsToProof(proof, options.ppOptions || {});
    }
  ...
  async verify(framed, options) {
    ...
    // TODO: Once multiple proofs are supported, we should instead filter
    // the proofs we're checking to those with the expectedProofPurpose.
    if(options.proofPurpose) {
      // Ensure that proofPurpose is the proofPurpose indeed that we expect
      if(!proof.proofPurpose === options.proofPurpose.uri) {
        throw new Error(
          'proof\'s proofPurpose does not match the expectedProofPurpose');
      }
      const ppVerified = options.proofPurpose.verify(
        verifyData, proof, options.ppOptions || {});
      // Okay now ensure that the proofPurpsoe validation passes
      if(!ppVerified) {
        return false;
      }
    } else if (!proof.proofPurpose === null) {
      // If a proofPurpose was not provided in options, we must be
      // operating in "backwards compatibility mode" with pre-proofPurpose.
      // Which means there shouldn't be a proofPurpose on the options either.
      throw new Error(
        'options.proofPurpose not provided but proofPurpose found on proof');
    }
